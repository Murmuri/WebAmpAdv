...
AudioLinks defaultAudioLinks[] = {
    ...
    {"LS4 Radio Continental 590", "http://edge05.radiohdvivo.com:9126/live"},
    ...
    {"Kniga FM (Russian)", "http://bookradio.hostingradio.ru:8069/fm"},
...
void getLinksList()
{
  ...
  while (file.available() && linksCount < MAX_LINKS)
  {
    ...
    if (c == '\n' || c == '\r')
    {
      ...
      if (currentLine.length() > 0)
      {
        ...
        if (commaIndex > 0)
        {
          String name = currentLine.substring(0, commaIndex);
          String url = currentLine.substring(commaIndex + 1);
  ...
  if (currentLine.length() > 0 && linksCount < MAX_LINKS)
  {
    currentLine.trim();
