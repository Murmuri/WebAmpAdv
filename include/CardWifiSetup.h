...
#define MAX_NETWORKS 10     // Maximum number of networks displayed
...
// Auxiliary function for calculating the hash of credentials
...
void displayWiFiInfo() {
    ...
    M5Cardputer.Display.drawString("WiFi connect", 35, 1);
...
String scanAndDisplayNetworks() {
    ...
    // Stores and organizes the networks
    networks.clear();
...
void connectToWiFi() {
    // Configure WiFi mode and power saving mode.
    WiFi.mode(WIFI_STA);
    esp_wifi_set_ps(WIFI_PS_MAX_MODEM);
    
    // Attempts to retrieve saved credentials
    ...
    // Verifies the hash of the credentials
    ...
    if (validCredentials) {
        ...
        while (millis() - startTime < WIFI_TIMEOUT) {
            ...
            if (M5Cardputer.BtnA.isPressed()) {
                ...
                preferences.end();
                M5Cardputer.Display.clear();
                M5Cardputer.Display.drawString("Memory erased.", 1, 60);
                delay(1000);
                ESP.restart();
                return;
            }
    ...
    // If it didn't connect, start the setup process
    M5Cardputer.Display.clear();
    M5Cardputer.Display.drawString("Settings WiFi", 1, 1);
